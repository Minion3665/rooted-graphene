...
# Find latest magisk version here: https://github.com/Minion3665/Magisk/releases, or:
# curl --fail -sL -I -o /dev/null -w '%{url_effective}' https://github.com/Minion3665/Magisk/releases/latest | sed 's/.*\/tag\///;'
# renovate: datasource=github-releases packageName=Minion3665/Magisk versioning=semver-coerced
...
function downloadAndroidDependencies() {
  ...
  if ! ls ".tmp/magisk-$MAGISK_VERSION.apk" >/dev/null 2>&1 && [[ "${POTENTIAL_ASSETS['magisk']+isset}" ]]; then
    curl --fail -sLo ".tmp/magisk-$MAGISK_VERSION.apk" "https://github.com/Minion3665/Magisk/releases/download/$MAGISK_VERSION/app-release.apk"
  fi
...
function findLatestVersion() {
  ...
  if [[ "$MAGISK_VERSION" == 'latest' ]]; then
    MAGISK_VERSION=$(curl --fail -sL -I -o /dev/null -w '%{url_effective}' https://github.com/Minion3665/Magisk/releases/latest | sed 's/.*\/tag\///;')
  fi
